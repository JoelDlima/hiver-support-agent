...
- **Source:** `data\raw\twcs.csv` (2,811,774 rows; TWCS only, PS-strict)
...
- **Downstream artifacts:** `data\processed\virgin_kb.csv` (27,172 rows, template-cap K=5) + `data\processed\virgin_inbound_pool.csv` (37,444 rows)
- **Tooling:** `.venv\Scripts\python.exe` (`PYTHONPATH=.`), pandas chunk scan (`chunksize=200k`); builder `scripts/build_virgin_kb.py`; intents `src/virgin_intents.py`
- **Date of inspection:** 2026-09-10

## 1. Schema

Same 7-column TWCS schema as Apple slice (`docs/research/datasets/data_quality.md` §1). Observed on Virgin union:
...
- Repro: `$env:PYTHONPATH="."; & ".venv\Scripts\python.exe" "scripts\build_virgin_kb.py"` → `STATS outbound=27172 inbound_pool=37444`. All paths under `.` only.
