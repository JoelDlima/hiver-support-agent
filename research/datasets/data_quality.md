...
- **Source:** `C:\Hiver\data\raw\twcs.csv` (2,811,774 rows)
...
- **Downstream sample:** `C:\Hiver\data\intermediate\apple_sample.csv` (5,000 rows, random `seed=42`, UTF-8, full schema)
- **Tooling:** `C:\Hiver\.venv\Scripts\python.exe`, pandas chunk scan (`chunksize=200k`); script `scripts/apple_inspect_tmp.py`
...
- Repro: re-run `C:\Hiver\.venv\Scripts\python.exe scripts/apple_inspect_tmp.py` (full-scan union → 200k `seed=42` analysis → `STATS_JSON` + 5k sample). All paths under `C:\Hiver` only.
