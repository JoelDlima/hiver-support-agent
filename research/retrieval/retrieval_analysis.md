...
## 1. Corpus facts (measured on `C:\Hiver\data\raw\twcs.csv`, not assumed)
...
8. **Persist:** `C:\Hiver\data\indexes\` — `tfidf_vectorizer.pkl`, `tfidf_matrix.npz`, `nn_index.pkl`, `doc_ids.csv`, `index_meta.json` (+ later `minilm_embeddings.npy` / `faiss_flat.ip`). Rebuild via `C:\Hiver\scripts\build_retrieval_baseline.py`.

## 5. Proof a vector DB is not needed (measured, this machine, CPU-only)

`C:\Hiver\scripts\build_retrieval_baseline.py` on the full 106,860 AppleSupport replies, deps = pandas + scikit-learn + scipy only:
...
**No vector database. Ship TF-IDF/BM25 + MiniLM dense hybrid (RRF-fused, top-k=5, cross-encoder rerank, `tweet_id` citations), persisted as local files in `C:\Hiver\data\indexes`.** Proven today: full-corpus repro in 17 s on CPU for $0.
