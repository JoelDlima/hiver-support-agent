...
| A final | 0.433 | 0.443 | **0.368** | 0.500 | **0.424** | 1.000 | 0.133 |
Deltas final−other: vs D acc +0.216/macro +0.411/esc_F1 +0.424; vs C acc −0.084/macro −0.104/esc_F1 +0.424; vs B acc ±0.000/macro ±0.000/esc_F1 +0.046/grounded +1.000.

## Which components matter
1. **Escalation rules head (dominant for safety).** C has retrieval but no rules → esc F1 0.000. Adding the 4-trigger head (B/A) → 0.378/0.424. The rules, not retrieval, create escalation capability.
2. **Retrieval (precision modulator + grounding).** A vs B isolates it (classifier identical → intent Δ 0.000 by design): esc_P 0.233→0.368, esc_F1 +0.046, unresolvable 0.833→0.133, grounded 0.000→1.000. Without passages `no_grounding` is always true → over-escalation (R=1.0, P=0.233); with passages the gate calibrates (P up, R 1.0→0.5). Retrieval buys **precision + audit trail (passage IDs)**, not intent accuracy.
