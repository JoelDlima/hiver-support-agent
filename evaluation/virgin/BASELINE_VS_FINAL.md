...
Date: 2026-09-10. Sources (read-only): `evaluation/virgin/golden_v1.csv` (weak-200) + `evaluation/virgin/golden_human_200.csv` (human-200: 60 manual-style + 140 rulebook-assisted, 41/200 intent flips, weak-vs-human acc 0.795 κ 0.772).
Systems: trivial = generic canned, never-escalate; simple = virgin keyword rules + virgin NN top-1; final = TF-IDF LogReg (`models/intent_virgin.pkl`, 30k weak, train-subset acc 0.949 optimistic) + virgin NN k=5 + virgin templates + brand-aware 4-trigger rules (rail SAFETY_ADDONS, money_intents).
...
| simple (virgin keyword + top-1) | 1.000 | 1.000 | 1.000 | 0.179 | 0.303 | 2.33 | 0.015 |
| final (TFIDF-LogReg + virgin NN k=5 + template + rules) | 0.795 | 0.793 | 0.500 | 0.893 | 0.641 | 4.21 | 0.935 |
Deltas final−simple: acc -0.205, macroF1 -0.207, esc_F1 +0.338.
...
| simple (virgin keyword + top-1) | 0.795 | 0.801 | 1.000 | 0.135 | 0.238 | 2.33 | 0.015 |
| final (TFIDF-LogReg + virgin NN k=5 + template + rules) | 0.660 | 0.672 | 0.580 | 0.784 | 0.667 | 4.21 | 0.935 |
Deltas final−simple: acc -0.135, macroF1 -0.129, esc_F1 +0.429.

## C. Per-intent F1 on human-200 (simple vs final; support in brackets)
| Intent | simple F1 | final F1 |
|---|---|---|
| delay_claim | 0.870 (24) | 0.791 (24) |
| ticket_change_refund | 0.762 (23) | 0.682 (23) |
| timetable_platform | 0.791 (21) | 0.667 (21) |
| lost_property | 0.857 (16) | 0.750 (16) |
| complaint_service | 0.714 (23) | 0.596 (23) |
| fare_ticketing | 0.947 (18) | 0.774 (18) |
| accessibility_assistance | 0.848 (14) | 0.500 (14) |
| howto_guidance | 0.703 (17) | 0.667 (17) |
| support_access_followup | 0.882 (15) | 0.811 (15) |
| other_out_of_scope | 0.640 (29) | 0.486 (29) |

## Interpretation (incl. simple-can-win honesty)
- **§B is the headline; §A is circularity demonstration.** Weak labels ARE the keyword rules' output, so simple scores ~1.0 tautologically. Final's LogReg generalizes off 30k weak bootstraps → lower vs weak labels by construction. This is expected, not a regression.
...
- Weak-200 numbers (§A) flatter simple (~1.0) and punish final for generalizing — never quote §A without the circularity warning. Train-subset acc 0.949 (vs weak) is equally circular.
