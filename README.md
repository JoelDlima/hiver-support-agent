...
|---|---|---|---|---|
| weak-200 (circular — do not cite) | trivial / simple / final | 0.095/0.017 – 0.975/0.974 – 0.785/0.786 | 0.000 – 0.303 – 0.625 | 0.000 – 0.015 – 0.935 |
| **human-200 (headline)** | trivial / simple / final | 0.075/0.014 – 0.790/0.796 – **0.670/0.685** | 0.000 – 0.238 – **0.652** (P 0.558 R 0.784) | 0.000 – 0.015 – **0.935** (mean 4.21) |

Baselines: **trivial** (majority + canned, never-escalate) vs **simple** (virgin keyword + top-1 copy) vs **final** (TF-IDF LogReg + virgin NN k=5 + template + rail rules). Final trades intent (−0.120 acc vs simple — weak-label ceiling) for escalation (+0.414 esc_F1) + groundedness. Safety recall **1.000 on n=3 human legal_safety (tiny — gate NOT claimable)**; money recall 0.800 (16/20); esc-vs-human acc 0.845 κ 0.556; mined-slice escalation rate safety 1.000 (n=20) / money 0.575 (n=40, unlabelled — coverage only). Judge = heuristic offline + LLM hook, **gated (wκ ≥0.60 + safety-recall ≥0.90 to ship, else advisory)** — keyed study 2026-09-11 (Groq qwen, n=30): verdict κ=0.253, groundedness wκ=0.060 → **gate correctly holds: advisory-only**. See `evaluation/virgin/JUDGE_AGREEMENT.md` + `evaluation/virgin/LLM_JUDGE_30.md` + `docs/ANNOTATION_PROTOCOL.md`. Failures F1–F7 exact-text re-probe 2026-09-11: 9 PASS / 0 PARTIAL / 0 FAIL (F2 still-running, F4a packed→complaint remap, F5 language gate, F6/F7 money gates all green; regression tests `tests/test_virgin_fixes.py`) — see `evaluation/virgin/FAILURE_TESTS.md`. Index 27,172 docs, build 0.5s, p50 7.1ms; Apple end-to-end p50 180ms/p95 192ms. Repro: eval+judge+21 tests ≈25s CPU. Full report: `docs/REPORT_VIRGIN_6PAGE.md` §1–6 (framing/not-built, baselines + Apple transfer row, top-5 failures, misleading, next-week incl. Groq, decisions pointer). Re-verified 2026-09-11 post-fix: `train_virgin.py` → `run_virgin_eval.py` + `run_virgin_judge.py` re-ran clean (weak-vs-human acc 0.795 κ 0.772); numbers match `results_human200.csv`/`BASELINE_VS_FINAL.md` exactly. Weak-200 simple is 0.975 not 1.000 — keyword rules changed after the weak golden froze (disclosed in BASELINE doc).
